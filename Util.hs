...
checkExamples :: Eq o1 => Show o1 => Eq o2 => Show o2 => (input -> o1) -> o1 -> (input -> o2) -> o2 -> input -> IO ()
checkExamples part1 expected1 part2 expected2 inputs = do
    let p1 = part1 inputs
    let p2 = part2 inputs
    if p1 == expected1
        then if p2 == expected2
            then putStrLn "test with example inputs passed"
            else putStrLn ("test with example inputs failed for part 2: " ++ show p2 ++ " /= " ++ show expected2)
        else
            putStrLn ("test with example inputs failed for part 1: " ++ show p1 ++ " /= " ++ show expected1)
